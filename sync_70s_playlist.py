...
def get_client_secret_path():
    # Check Desktop first for new downloads
    desktop_files = glob.glob(os.path.expanduser("~/Desktop/client_secret*.json"))
    if desktop_files:
...
def get_authenticated_service():
    ...
    if not client_secret_file:
        print("\n[ERROR] No client_secret.json found in ~/Desktop or ~/Downloads!", flush=True)
        return None
...
def resolve_video_id(title, movie, singers, channel):
    ...
    try:
        ...
        print(f"   [!] Search error for {title}: {e}", flush=True)
    return None
...
def main():
    ...
    print("\n==========================================", flush=True)
    print("🎉 1970s PLAYLIST CREATED & SYNCED SUCCESSFULLY!", flush=True)
    print(f"1970s Playlist URL ({success_70s}/100 songs): {pl70_url}", flush=True)
    print("==========================================", flush=True)
